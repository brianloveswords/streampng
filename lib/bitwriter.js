...
function BitWriter() { this.initialize.apply(this, arguments) }
BitWriter.prototype = {
  initialize: function initialize(length, endianness) {
  ...
  write: function write(data, opts) {
    ...
    if (opts) {
      var methods = (opts.signed || !opts.unsigned)
        ? this._methods.bySize['signed']
        : this._methods.bySize['unsigned']
      type = methods[opts.size];
    }
  ...
  writeString: function (string, opts) {
    var nullbyte = '\u0000';
  ...
  writeRaw: function (array) {
    for (var n = 0; n < array.length; n++) {
      this._buffer.writeUInt8(array[n], this._offset++);
    }
  },

  _generate: function generate() {
