...
function StandardRGBColourSpace(data) {
  ...
  this.renderingIntent = p.eat(1).readUInt8(0);
}
...
function SignificantBits(data, header) {
  ...
  if (colour === 0)
    ...
    this.red = p.eat(1).readUInt8(0);
    this.green = p.eat(1).readUInt8(0);
    this.blue = p.eat(1).readUInt8(0);
  }

  else if (colour === 4) {
    this.greyscale = p.eat(1).readUInt8(0);
    this.alpha = p.eat(1).readUInt8(0);
  }
