...
Chunk.prototype.checkCRC = function () {
  var calculated = crc32(Buffer.concat([this._rawType, this._rawData]));
  this.crcCheck = calculated;
  return bufferEqual(this.crc, calculated);
}
...
function CompressedText(data) {
  ...
  this.type = 'zTXt';
  this.keyword = p.eatString();
  this.compressionMethod = p.eatInt(1);
...
function Transparency(data, header) {
  ...
  if (colour === 0)
    this.grey = p.eatInt(2);

  else if (colour === 2) {
    this.red = p.eatInt(2);
    this.green = p.eatInt(2);
    this.blue = p.eatInt(2);
  }
...
function Dimensions(data) {
  ...
  this.pixelsPerUnit = new Point(p.eatInt(4), p.eatInt(4));
  this.unitSpecifier = p.eatInt(1);
}
...
function StandardRGBColourSpace(data) {
  ...
  this.renderingIntent = p.eatInt(1);
}
...
function SignificantBits(data, header) {
  ...
  if (colour === 0)
    ...
    this.red = p.eatInt(1);
    this.green = p.eatInt(1);
    this.blue = p.eatInt(1);
  }

  else if (colour === 4) {
    this.greyscale = p.eatInt(1);
    this.alpha = p.eatInt(1);
  }
...
function SuggestedPalette(data) {
  ...
  this.palette = p.eatRest({ chunkSize: chunkSize }).map(function (entry) {
    ...
    return {
      red: pp.eatInt(colourSize),
      green: pp.eatInt(colourSize),
      blue: pp.eatInt(colourSize),
      alpha: pp.eatInt(colourSize),
      frequency: pp.eatInt(2)
    }
  });
}
...
function LastModified(data) {
  var p = new Parser(data);
  this.type = 'tIME';
  this.year = p.eatInt(2);
  this.month = p.eatInt(1);
  this.day = p.eatInt(1);
  this.hour = p.eatInt(1);
  this.minute = p.eatInt(1);
  this.second = p.eatInt(1);
